...
class KernelManager(ConnectionFileMixin):
    ...
    async def _async_start_kernel(self, **kw: t.Any) -> None:
        ...
        `**kw` : optional
             ...
             and launching the kernel (e.g. Popen kwargs).
        """
        kernel_cmd, kw = await ensure_async(self._async_pre_start_kernel(**kw))

        # launch the kernel subprocess
        self.log.debug("Starting kernel: %s", kernel_cmd)
        await ensure_async(self._async_launch_kernel(kernel_cmd, **kw))
        await ensure_async(self._async_post_start_kernel(**kw))
    ...
    async def _async_finish_shutdown(
        ...
        try:
            ...
            await ensure_async(self._async_send_kernel_sigterm())
        ...
        try:
            ...
            await ensure_async(self._async_kill_kernel(restart=restart))
        else:
    ...
    async def _async_shutdown_kernel(self, now: bool = False, restart: bool = False) -> None:
        ...
        if self.has_kernel:
            await ensure_async(self._async_interrupt_kernel())

        if now:
            await ensure_async(self._async_kill_kernel())
        else:
            await ensure_async(self._async_request_shutdown(restart=restart))
            # Don't send any additional kernel kill messages immediately, to give
            # the kernel a chance to properly execute shutdown actions. Wait for at
            # most 1s, checking every 0.1s.
            await ensure_async(self._async_finish_shutdown(restart=restart))
    ...
    async def _async_restart_kernel(
        ...
        await ensure_async(self._async_shutdown_kernel(now=now, restart=True))
    ...
    async def _async_interrupt_kernel(self) -> None:
        ...
        if self.has_kernel:
            ...
            if interrupt_mode == "signal":
                await ensure_async(self._async_signal_kernel(signal.SIGINT))
    ...
    async def _async_wait(self, pollinterval: float = 0.1) -> None:
        ...
        while await ensure_async(self._async_is_alive()):
